...
export default function scroll(event: PointerEvent) {
    const data = {
        menu: (event.target as HTMLElement).parentElement,
        initialY: event.pageY,
        initialScrollTop: (event.target as HTMLElement).parentElement?.scrollTop,
    ...
    data.updateVelocity = (event) => updateVelocity(data, event);

    (event.target as HTMLElement).setPointerCapture(event.pointerId);

    // Bind for the event listeners
    data.pointerMove = (event) => pointerMove(data, event);
    data.pointerUp = (event) => pointerUp(data, event);
...
function updateVelocity(data: Data, event: PointerEvent) {
    const time = Date.now();
    const velocity = ((event.pageY - data.previousY)
                      / (time - data.previousTime));
    if (data.velocity)
        data.velocity = (velocity + data.velocity) / 2
    else
...
function pointerMove(data: Data, event: PointerEvent) {
    data.menu.scrollTop = data.initialScrollTop + data.initialY - event.pageY;
    data.updateVelocity(event);
}


function pointerUp(data: Data, event: PointerEvent) {
...
function endInertia(menu: HTMLElement, downwards: boolean | null, v: number | null, t0?: number, t?: number) {
    if (v === null) {
