...
export default function drag(event: PointerEvent) {
...
function pointerMove(clone: Clone, event: PointerEvent) {
    if (clone.currentTrack) {
    ...
    let elemBelow = document.elementFromPoint(event.pageX, event.pageY) as HTMLElement;
