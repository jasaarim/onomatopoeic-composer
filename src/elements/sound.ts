...
export function newSound(name: string, files: Files) {
    const sound = document.createElement('div') as Sound;
    const audio = files.audio ? createSoundAudio(files.audio) : null;
...
function createSoundAudio(source: string) {
...
function createAddButton(audio: HTMLAudioElement | null) {
