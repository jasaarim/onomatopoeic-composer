...
export interface Player extends HTMLElement {
  ...
  clearAudioCxt: () => void
  applyActiveSounds: (action: (sound: ActiveSound) => void) => void
  warmedUp?: boolean
  warmupAudioCxt: () => void
  getATrack: () => Track
  audioEnd: () => number
...
export function initializePlayer (num: number, duration: number): void {
...
function setDuration (player: Player, seconds: number): void {
  if (seconds > 0) {
    ...
    const prevPosition = (player.start !== undefined) ? player.start / player.duration : 0
...
function getAudioCxt (player: Player): AudioContext {
  if (player.audioCxt === undefined) { player.audioCxt = newAudioCxt() }
...
function clearAudioCxt (player: Player): void {
  if (player.audioCxt != null) {
    player.audioCxt.close().catch(error => { throw error })
    delete player.audioCxt
  }
}

function applyActiveSounds (player: Player, action: (sound: ActiveSound) => void): void {
...
function warmupAudioCxt (player: Player): void {
